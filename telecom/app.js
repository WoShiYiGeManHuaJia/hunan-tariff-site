/* 中国电信资费专区 - 前端逻辑（vdx20260911a：上下线时间/退订方式/违约责任/在网要求/销售渠道/其他说明补齐，适用对象等上移主表；保留零元筛选、按钮震动、无声音） */
"use strict";
const DATA = "./data/";
...
function itemHtml(it, idx) {
  ...
  const mainKeys = ["资费类型", "月费标准", "语音", "流量", "短信", "定向流量", "宽带", "有效期", "停售状态", "业务编码", "适用对象", "销售渠道", "上线日期", "下线日期", "退订方式", "违约责任", "在网要求"];
  const rows = detailRows(it);
  // 说明类字段（套餐内容/其他说明/其他收费）固定折叠为「其他说明」，所有业务统一展示折叠入口；适用对象/销售渠道/上下线时间等上移主表
  const NOTE_KEYS = ["套餐内容", "其他说明", "其他收费"];
